...
Variable ghost_list_show_map_full("ghost_list_show_map_full", "0", 0 , 1, "Show the maps actual name as opposed to map id.\n");

static std::map<std::string, const char *> map_id_to_name = {
...
public:
	...
	virtual void Paint(int slot) override {
		...
		if (ghost_list_show_map.GetBool()) {
			if (ghost_list_show_map_full.GetBool() && (map_id_to_name.find(engine->GetCurrentMapName().c_str()) != map_id_to_name.end())) {
				players.insert(Utils::ssprintf("%s (%s)", networkManager.name.c_str(), map_id_to_name[engine->GetCurrentMapName().c_str()]));
			} else {
		...
		for (auto &g : networkManager.ghostPool) {
			...
			if (ghost_list_show_map.GetBool()) {
				if (ghost_list_show_map_full.GetBool() && (map_id_to_name.find(g->currentMap.c_str()) != map_id_to_name.end())) {
					players.insert(Utils::ssprintf("%s (%s)", g->name.c_str(), map_id_to_name[g->currentMap.c_str()]));
				} else {
